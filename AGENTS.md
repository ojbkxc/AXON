...
├── android/            ❌ 空     # termux/ apk/ NDK/ 脚本
├── scripts/            ❌ 空     # 构建/安装/交叉编译脚本
├── tests/e2e/          ❌ 空     # E2E 测试
└── docs/               ❌ 空     # 文档
```
...
- **CI（`.github/workflows/ci.yml`）**：`rust` job（fmt --check / clippy -D warnings / test --workspace / release build / 二进制 < 50MB 断言）+ `ui` job（npm install / tsc --noEmit / vite build / 上传 dist artifact）。push 到 main 或 PR 触发。
...
- **Anthropic 工具消息格式**：`provider.rs` 的 AnthropicProvider 直接透传 `role: "tool"` 消息，但 Anthropic API 要求 `tool_result` content block 格式。多轮工具循环对 Anthropic 可能失败。需在 provider 层做消息格式转换。
...
- `ChatMessage::{system,user,assistant,tool}` 构造器；`ChatOptions::default()`。
...
- `StreamEvent` 7 变体：TextChunk/ThoughtChunk/ToolCallRequest/ToolCallResult/UsageUpdate/Error/Done（`#[serde(tag="type", rename_all="snake_case")]`）。
...
### P4 — v0.4 移动端适配
- [ ] `scripts/cross-android.sh`（NDK `aarch64-linux-android`）。
- [ ] 裁剪依赖、验证二进制 < 30MB、启动内存 < 150MB。
...
- [ ] 验收：`git push` 后 GitHub CI rust + ui 两 job 全绿；浏览器打开 `http://localhost:8080/ui/` 能看到管理界面并能创建对话（需先接入静态资源服务）。
