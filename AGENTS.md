...
- 2026-08-11 **修复 APK 打不开（4 个根因，全量修复）**：全量分析对比 Agora Android 工程,定位 APK 闪退的 4 个根因并修复。(1) **缺少 AppCompat 主题**(致命):`MainActivity extends AppCompatActivity` 但 manifest 没设 `android:theme` → `IllegalStateException` 闪退。修复:新建 `res/values/styles.xml` 定义 `Theme.Axon`(AppCompat.Light.DarkActionBar),manifest 加 `android:theme="@style/Theme.Axon"`。(2) **二进制无法执行**(致命):Android 10+ W^X 限制,`getFilesDir()` 下文件不能 `setExecutable` → `ProcessBuilder.start()` 权限拒绝。修复:改用 jniLibs 打包,二进制重命名 `libaxon.so` 放到 `src/main/jniLibs/arm64-v8a/`,运行时从 `nativeLibraryDir` 加载(系统自动设置可执行权限),manifest 加 `android:extractNativeLibs="true"`,build.gradle 加 `packaging { jniLibs { useLegacyPackaging = true } }`。(3) **前台服务通知权限**(Android 13+):`POST_NOTIFICATIONS` 未请求运行时权限。修复:MainActivity 添加运行时权限请求。(4) **前台服务类型权限**(Android 14):`FOREGROUND_SERVICE_DATA_SYNC` 未声明。修复:manifest 添加权限。同时改进 WebView 设置(MIXED_CONTENT_COMPATIBILITY_MODE、database enabled)、startForeground try-catch 防崩溃、增加 healthcheck 轮询到 12 秒。改造 `android-apk.yml` 为 Agora 风格 Build & Release workflow(tag `v*` 触发 + workflow_dispatch version input + get-version + build-android + release job)。重新打 tag v1.0.1 触发 CI 构建。**待 CI 全绿验证**。
